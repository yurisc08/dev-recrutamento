...
(function () {
  ...
  const RASCUNHO_LOCAL = 'cine1up:editor';
  ...
  function vazio() {
    return {
      id: null, slug: '', titulo: '', subtitulo: '', categoria: window.CINE1UP.CATEGORIAS[0],
      tags: [], capa_url: null, arte_cena: null, nota: null, ano: null, duracao: null,
      diretor: '', autor: 'Redação CINE 1UP', trailer_url: '', destaque: false,
  ...
  function atualizarCapa() {
    ...
    const url = window.CV.capaDe(
      Object.assign({}, atual, { slug: atual.slug || atual.titulo || 'cine1up' }),
      { w: 600 }
    );
  ...
  function montarPainel() {
    ...
    $('[data-campo="categoria"]').innerHTML =
      window.CINE1UP.CATEGORIAS.map(c => `<option>${c}</option>`).join('');
