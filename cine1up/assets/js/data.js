...
(function () {
  ...
  function capaDe(post, opcoes) {
    ...
    if (window.Art) {
      return window.Art.posterURL(post.slug || post.titulo || 'cine1up', Object.assign({
  ...
  const DEMO_POSTS = [
    {
      id: 'demo-0', slug: 'o-que-vem-por-ai-na-temporada',
      titulo: 'O que vem por aí na temporada',
      subtitulo: 'Um apanhado do que a redação está de olho nos próximos meses',
      categoria: 'Notícia', tags: ['agenda', 'estreias'],
      capa_url: null, nota: null, ano: null, duracao: null,
      diretor: null, autor: 'Redação CINE 1UP', destaque: false, status: 'publicado',
      publicado_em: '2026-09-10T12:00:00Z', views: 1240,
      corpo: 'Este é um texto de exemplo, para você ver como fica uma notícia publicada aqui.\n\nAs vitrines de **Em cartaz**, **Estreias** e **Séries em alta** se atualizam sozinhas pelo TMDB — você não precisa digitar nada para elas funcionarem. Esta editoria de **Notícia** é para o que você quiser escrever com as suas palavras: bastidor, opinião, apuração, agenda.\n\n> Apague esta matéria no painel em /admin.html quando publicar a primeira de verdade.'
    },
    {
      ...
      diretor: null, autor: 'Redação CINE 1UP', destaque: true, status: 'publicado',
  ...
  const LS_KEY = 'cine1up:posts';
  const LS_SCORES = 'cine1up:scores';
  ...
  async function entrar(email, senha) {
    if (!sb) return { user: { email: email || 'demo@cine1up.local' }, demo: true };
  ...
  async function assinar(email) {
    if (!sb) { lsSet('cine1up:news:' + email, true); return true; }
