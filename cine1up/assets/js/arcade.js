/* ============================================================
   ...
   assumiu", raio ao comer a pílula (o volt do CINE 1UP) e os
...
(function () {
  ...
  function iniciarLabirinto() {
    ...
    let recorde = Number(localStorage.getItem('cine1up:maze') || 0);
    ...
    const lab = new window.Labirinto(cv, {
      ...
      aoMorrer: pontos => {
        if (pontos > recorde) {
          recorde = pontos;
          localStorage.setItem('cine1up:maze', String(pontos));
          if (recordeEl) recordeEl.textContent = String(recorde).padStart(6, '0');
        }
