...
(function () {
  ...
  function naoEncontrado() {
    ...
    $('[data-post-sub]').innerHTML =
      'O link pode ter mudado. <a class="link-fx" href="/criticas.html">Ver as críticas</a>.';
  ...
  function compartilhar(post) {
    document.addEventListener('click', e => {
      ...
      const txt = `${post.titulo} — CINE 1UP`;
  ...
  async function init() {
    ...
    /* --- cabeçalho --- */
    document.title = post.titulo + ' — CINE 1UP';
    ...
    $('[data-post-tags]').innerHTML = tags
      .map(t => `<a class="pill" href="/criticas.html?q=${encodeURIComponent(t)}">#${window.UI.esc(t)}</a>`).join('');
